...
background_image = pygame.transform.scale(pygame.image.load("background_image.png"), (SCREEN_WIDTH, SCREEN_HEIGHT))
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == pyagame.KEYDOWN and event.key == pyagame.K_x):
            running = False
    if not won:
        keys=  pygame.key.get_presseed()
        x_change= (keys[pygame.K.RIGHT]- keys[pygame.K_LEFT]) * MOVEMENT_SPEED
